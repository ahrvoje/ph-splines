...
def test_exact_reversal_on_line():
    with pytest.raises(ReversalError):
        CubicPHSplineOpen([[0.0, 0.0], [2.0, 0.0], [1.0, 0.0]])
...
def test_self_intersection_direct():
    ...
    from ph_spline.predicates import polyline_self_intersection
...
def test_chord_ratio_too_small():
    with pytest.raises(NumericalPrecisionError):
        CubicPHSplineOpen([[0.0, 0.0], [1e-14, 1e-15], [1.0, 0.5], [1.5, 1.5]])


def test_overflow_during_normalization():
    with pytest.raises(NumericalPrecisionError):
        CubicPHSplineOpen([[-1e308, 0.0], [1e308, 1e300], [1e308, 2e300]])


def test_clockwise_data_accepted():
    CubicPHSplineOpen(circle_points(cw=True))


def test_counterclockwise_data_accepted():
    CubicPHSplineOpen(circle_points(cw=False))
